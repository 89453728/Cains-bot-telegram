...
def dep(update: Update, context: CallbackContext) -> None:
    ...
    if (usuario == -1):
        usuario = User(getMaxID(),username,0,0,0,0,0)
        newUser(usuario)
...
def dar(update: Update, context: CallbackContext): 
    ...
    if (usuario1 == -1):
        usuario1 = User(getMaxID(),username1,0,0,0,0,0)
        newUser(usuario1)
    usuario2 = getUser(username2)
    if (usuario2 == -1):
        usuario2 = User(getMaxID(),username2,0,0,0,0,0)
        newUser(usuario2)
    ammount = usuario1.give(cantidad)
    if (ammount != -1):
        usuario2.get(ammount)
    else:
        update.message.reply_text("** NO TIENES SUFICIENTES BLANCHUS ** 😔")
        return
    saveUser(usuario1)
    saveUser(usuario2)
...
def robar(update: Update, context: CallbackContext):
    ...
    if (canbe == -1):
        update.message.reply_text("** NO PUEDES ROBAR A ESTE USUARIO ** \n Iyo, que le han robao hace menos de 12 horas, dale tregua, no?")
        return 
    ...
    if(estado == TRIUNFO):
        ...
        if(cant == -1):
            update.message.reply_text("** TE HA ATRAPADO LA POLI ** \n @"+username1+" ha sido atrapado por la policia y debe pagarle una indemnizacion a @"+username2+" pero no tiene dinero en la cartera asi que se le retirara del banco XD")
            x = int(random.random()*usuario1.banco)
            usuario1.ret(x)
            cant = usuario1.give(x)
            usuario2.get(cant)
            update.message.reply_text("** INDEMNIZACION EXITOSA! ** \n@"+username1+" ha indemnizado a @"+ username2 + " con " + str(cant) + " 📀 Blanchus")
            return 
        usuario2.get(cant)
        saveUser(usuario1)
        saveUser(usuario2)
    else:
        update.message.reply_text("** EL LADRON SE ESCAPA ** \n@"+username1 + " se ha dado a la fuga antes de que llegue la poli al haber fracasado")
